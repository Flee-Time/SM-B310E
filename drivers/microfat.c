/*
 * B310E-OS — drivers/microfat.c
 *
 * Minimal READ-ONLY FAT32 layer, ported VERBATIM from fpdoom's microfat.c
 * (fpdoom reference clone, Unlicense / public domain — freely copyable).
 * All fpdoom code and comments are preserved; only the libc/stdio/sector-I/O
 * glue was adapted for the B310E-OS -nostdlib kernel.
 *
 * READ-ONLY: FAT_WRITE stays 0, so the fpdoom write-path code below
 * (#if FAT_WRITE blocks) is compiled out and fatfile.c is NOT ported —
 * it needs malloc/free, which the B310E-OS bump allocator does not
 * provide. See microfat.h for the full rationale.
 *
 * HOST_TEST: this file is NOT part of the host test build (HOSTTEST_SRCS
 * is kernel + tests only), but the #ifndef HOST_TEST guard below keeps it
 * host-compilable anyway: the FAT_READ_SYS sector hook (which pulls in
 * drivers/sdio.h and the hardware block-read) is only defined on the
 * target. Without FAT_READ_SYS, fat_read_sec() is a prototype only, so any
 * host caller would need to inject its own FAT_READ_SYS + FAT_WRITE_SYS.
 *
 * SAFETY: pure 32-bit arithmetic, no allocation, no hardware writes; all
 * waits are bounded by sector-read failure (NULL return) and the on-disk
 * FAT structures. Never writes the 0x8cxxxxxx pinmux (or any register).
 */

#include <stddef.h>          /* size_t, for the arm_helpers externs below */

/* fpdoom pulled these from <string.h>; the B310E-OS kernel is -nostdlib and
 * provides its own at link time in drivers/arm_helpers.c. Declared here
 * (NOT in microfat.h) only for the functions this file actually uses:
 * memcmp (BPB type check), strlen (name lookups), memcpy (FAT_WRITE
 * dir-creation path — inert while FAT_WRITE is 0). */
extern void *memcpy(void *dst, const void *src, size_t n); /* arm_helpers.c */
extern size_t strlen(const char *s);                        /* arm_helpers.c */
extern int memcmp(const void *a, const void *b, size_t n);  /* arm_helpers.c */

#ifndef HOST_TEST
/* Sector source: drivers/sdio.c. sdio_read_block(uint32_t sector,
 * uint8_t *buf) returns 0 on success, non-zero on error; `sector` is the
 * raw LBA as presented by fat_read_sec() (0 = MBR), and sdio_shl is
 * applied inside sdio_read_block for the card class (SDHC vs SDSC).
 * Same binding as fpdoom sdboot/main.c:22-23. */
#include "sdio.h"
#define FAT_READ_SYS \
...
#if FAT_DEBUG
/* fpdoom used <stdio.h> + fprintf(stderr, ...) here; B310E-OS has no
 * stdio (and kprintf pulls in div helpers that the FAT layer should not
 * depend on), so FAT_LOG is a no-op in this port and FAT_DEBUG defaults to
 * 0 in microfat.h. ENTRY_FMT/ENTRY_POS are kept so the call sites compile
 * either way; rewire FAT_LOG to kprintf() if debug output is ever wanted. */
#define FAT_LOG(...) (void)0
static inline
