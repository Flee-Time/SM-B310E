/*
 * B310E-OS — drivers/microfat.h
 *
 * Minimal READ-ONLY FAT32 layer, ported VERBATIM from fpdoom's microfat.h
 * (fpdoom reference clone, Unlicense / public domain — freely copyable).
 * The public API and the fatdata_t struct layout are byte-identical to
 * fpdoom.
 *
 * Public API:
 *   fat_init(fatdata_t*, int part_num)   — parse MBR + FAT32 BPB
 *   fat_find_path(fatdata_t*, name)      — resolve "/dir/file" to an entry
 *   fat_find_name(fatdata_t*, clust, name, len)
 *   fat_read_simple(fatdata_t*, clust, buf, size)
 *   fat_dir_clust / fat_next_clust / fat_enum_name / fat_entry_clust
 *
 * READ-ONLY: FAT_WRITE stays 0 (the default). fpdoom's write path
 * (fat_create_name / fat_make_dir / fat_delete_entry / fat_rmdir_check) is
 * not compiled, and fatfile.c is intentionally NOT ported: it needs
 * malloc/free (heap-managed file handles + cluster chains) which the
 * B310E-OS bump allocator does not provide. The boot path needs nothing of
 * it: fat_init(part 0) -> fat_find_path("rockbox/rockbox.elf") ->
 * fat_read_simple() into a fixed buffer.
 *
 * Sector I/O: fat_read_sec() is an inline hook defined only when
 * FAT_READ_SYS is defined. microfat.c defines FAT_READ_SYS to
 * drivers/sdio.c's sdio_read_block() BEFORE including this header (the
 * same pattern as fpdoom sdboot/main.c). Under HOST_TEST the hook is not
 * defined, so the inline body does not compile (prototype only).
 *
 * SAFETY: pure 32-bit arithmetic (no 64-bit ops — nothing pulls in the
 * libgcc div/mul helpers), zero dynamic allocation, no hardware access.
 * Every loop is bounded by the on-disk FAT structures and by sector reads
 * that return NULL on I/O error — a corrupt card cannot hang the
